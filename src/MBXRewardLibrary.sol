...
library MBXReward {
    ...
    function updateReward(
        ...
        rewardData.rewardPerTokenStored = rewardPerToken(rewardData, totalSupply);
        rewardData.lastUpdateTime = lastTimeRewardApplicable(rewardData.periodFinish);
        userRewardData.rewards = earned(rewardData, userRewardData, userBalance);
        userRewardData.userRewardPerTokenPaid = rewardData.rewardPerTokenStored;
        return (rewardData, userRewardData);
    }

    function rewardPerToken(RewardData memory rewardData, uint256 totalSupply) internal view returns (uint256) {
        if (totalSupply == 0) {
        ...
        return rewardData.rewardPerTokenStored
            + (
                (
                    (lastTimeRewardApplicable(rewardData.periodFinish) - rewardData.lastUpdateTime) * rewardData.rewardRate
                        * SCALING_FACTOR
                ) / totalSupply
            );
    }

    function earned(RewardData memory rewardData, UserRewardData memory userRewardData, uint256 userBalance)
        internal
        view
        returns (uint256)
    {
        return (userBalance * (rewardPerToken(rewardData, userBalance) - userRewardData.userRewardPerTokenPaid))
            / SCALING_FACTOR + userRewardData.rewards;
    }
